defmodule SwarmCode.Development.PersistedSession do
  ...
  def run_for_test(boot_config) do
    unless Mix.env() == :test, do: Mix.raise("Test boot configuration requires MIX_ENV=test")
    launch(boot_config)
  end

  defp launch(test_boot) do
    unless System.argv() == [], do: Mix.raise("Saved session accepts no command-line arguments")

    unless :init.get_argument(:noinput) != :error and :prim_tty.isatty(:stdin) == true and
             :prim_tty.isatty(:stdout) == true and
             System.get_env("TERM") not in [nil, "", "dumb"],
           do: Mix.raise("SAVED DEV SESSION requires a real terminal and -noinput")
    ...
    try do
      ...
      receive do
        ...
        {:DOWN, ^owner_monitor, :process, ^owner, _} ->
          Mix.raise("Saved terminal failed")

        {:DOWN, ^supervisor_monitor, :process, ^supervisor, _} ->
          Mix.raise("Saved session failed")
      end
    after
  ...
  defp selection_options!(id) do
    case Ecto.UUID.cast(id) do
      ...
      _ -> Mix.raise("SWARM_CONVERSATION must be latest, new, or a UUID")
    end
  end
  ...
  defp private_directory! do
    ...
    case File.mkdir(dir) do
      ...
      _ ->
        Mix.raise("Unable to create saved-session socket directory")
    end
  end
