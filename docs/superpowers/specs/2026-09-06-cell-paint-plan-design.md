...
This is the next implementation slice toward a visible desktop-style TUI. It
...
The current reducer and Projector already produce responsive Carbon Scenes,
but no code proves which cells their nested blocks occupy. A renderer-specific
...
region_id and optional control_id. Diagnostics contain only closed reason atoms
...
32 MiB. These are candidate-development limits, not changed canonical data limits.
...
replacement, style inheritance, cursor and action rectangles. Exercise all sixteen
