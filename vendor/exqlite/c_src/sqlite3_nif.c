...
static ErlNifResourceType *guard_resource_type = NULL;
...
exqlite_deserialize(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    connection_acquire_lock(conn);
#ifdef SWARM_GUARD_TEST
    if (conn->guard_resource) {
        connection_release_lock(conn);
        return guard_error(env, "guard_operation_refused");
    }
#endif
...
#ifdef SWARM_GUARD_TEST
    guard_resource_type = enif_open_resource_type(env, NULL, "scratch_guard",
      guard_resource_destructor, ERL_NIF_RT_CREATE, NULL);
    if (!guard_resource_type) return -1;
...
static ErlNifFunc nif_funcs[] = {
  ...
  {"guard_resource_identity", 1, guard_resource_identity_nif, ERL_NIF_DIRTY_JOB_IO_BOUND},
  {"guard_connection_identity", 1, guard_connection_identity_nif, ERL_NIF_DIRTY_JOB_IO_BOUND},
  {"guard_resource_close", 1, guard_resource_close_nif, ERL_NIF_DIRTY_JOB_IO_BOUND},
  {"guard_close_attested", 1, guard_close_attested_nif, ERL_NIF_DIRTY_JOB_IO_BOUND},
  {"guard_counts", 0, guard_counts_nif, ERL_NIF_DIRTY_JOB_IO_BOUND},
#endif
