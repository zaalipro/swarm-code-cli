defmodule SwarmCode.LLM.OpenAI do
  @moduledoc """
  ...
  alias SwarmCode.LLM.{Chunks, Efforts, HTTP, Request, Result, SSE, ToolArgs}
  ...
  defdelegate effort?(provider), to: SwarmCode.LLM.ProviderCaps

  @doc false
  defdelegate remember_no_effort(provider), to: SwarmCode.LLM.ProviderCaps

  @doc false
  def reset_caps, do: SwarmCode.LLM.ProviderCaps.reset()
  ...
  defp handle_event(data, acc, on_event, name) do
    case Jason.decode(data) do
      ...
      _other -> put_error(acc, "invalid JSON in SSE event", name)
    end
  end
