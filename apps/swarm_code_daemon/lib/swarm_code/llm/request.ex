defmodule SwarmCode.LLM.Request do
  ...
  @type t :: %__MODULE__{
          provider: SwarmCode.Providers.Provider.t() | nil,
          model: String.t() | nil,
          system: String.t(),
          messages: [message()],
          tools: [tool_spec()],
  ...
  what it means, `SwarmCode.LLM.Efforts.normalise_key/3` checks it against
  that list); anything else, nil included, is `"medium"`.
  """
  def effort(value) when is_binary(value) do
    if Regex.match?(SwarmCode.LLM.Efforts.key_format(), value), do: value, else: "medium"
  end
