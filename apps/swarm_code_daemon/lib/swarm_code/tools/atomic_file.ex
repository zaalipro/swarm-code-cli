defmodule SwarmCode.Tools.AtomicFile do
  @moduledoc """
  ...
  **Confined.** The target is validated against a root the *caller* supplies,
  ...
  `SwarmCode.Tools.Path.resolve/2` already refuses a link that escapes the root,
  ...
  knows what a conflict *means*, not under every write. `SwarmCode.Memory`, where
