  ...
  def description,
    do:
      "Replace an exact string in an existing text file, in place. old_string is matched " <>
        ...
        "for a larger one. The write is atomic, so a failed edit leaves the " <>
    ...
    case File.read(abs) do
      ...
      {:ok, content} ->
        if Grep.binary?(content) do
          ...
          edit(content, ctx.project_root, abs, rel, args, progress)
        end
    end
  end
