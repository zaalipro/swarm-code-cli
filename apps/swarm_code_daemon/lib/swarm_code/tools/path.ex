defmodule SwarmCode.Tools.Path do
  ...
  def real_path(path) do
    ...
    [anchor | components] = Elixir.Path.split(expanded)
    resolve_components(anchor, components, 0)
  end
  ...
  # Resolve from the root down. Resolving only the deepest link and appending
  # its tail misses links inside the target directory (alias/inner/secret).
  defp resolve_components(_base, _parts, hops) when hops > 40,
    do: {:error, :symlink_cycle}

  defp resolve_components(base, [], _hops), do: {:ok, base}

  defp resolve_components(base, ["." | rest], hops),
    do: resolve_components(base, rest, hops)

  defp resolve_components(base, [".." | rest], hops),
    do: resolve_components(Elixir.Path.dirname(base), rest, hops)

  defp resolve_components(base, [part | rest], hops) do
    candidate = Elixir.Path.join(base, part)

    case File.lstat(candidate) do
      {:ok, %{type: :symlink}} ->
        with {:ok, target} <- File.read_link(candidate) do
          # Do not expand the target: sub/alias/../file must traverse alias
          # before applying .. to its physical directory, exactly as the OS does.
          parts = String.split(target, "/", trim: true)
          anchor = if String.starts_with?(target, "/"), do: "/", else: base
          resolve_components(anchor, parts ++ rest, hops + 1)
        end

      {:ok, %{type: :directory}} ->
        resolve_components(candidate, rest, hops)
      ...
      {:error, :enoent} ->
        # A missing component cannot be traversed to reach its parent. Ordinary
        # new-file tails remain valid so atomic writes can create directories.
        if ".." in rest,
          do: {:error, :enoent},
          else: {:ok, Elixir.Path.join([candidate | rest])}
