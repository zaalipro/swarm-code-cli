defmodule SwarmCode.Domain.LLM.Efforts do
  @moduledoc """
  ...
  alias SwarmCode.Domain.LLM.{Anthropic, ProviderCaps, Request}
  ...
  def levels(nil, _model), do: defaults("openai_compatible", nil)
  ...
  def levels(provider, model) when is_map(provider) do
    ...
    cond do
      ...
      true ->
        defaults(Map.get(provider, :kind) || "openai_compatible", model)
    end
  end
  ...
  def presets do
    ...
    both = ["openai_compatible", "anthropic"]
